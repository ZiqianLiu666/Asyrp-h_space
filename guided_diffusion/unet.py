    ...
    def forward(self, x, timesteps, y=None, index=None, t_edit=400, hs_coeff=(1.0, 1.0), delta_h=None, ignore_timestep=False , use_mask=False):
        ...
        if index is not None:
            ...
            if timesteps[0] >= t_edit:
                # index 不为 None 且时间步大于 t_edit，会在这个 h 的基础上加上编辑模块（例如 DeltaBlock 产生的 delta_h）得到 h2，进而生成编辑后的图像；
                # use DeltaBlock
...
class UNet_h_encoder(nn.Module):
    def __init__(self, unet_model):
        super().__init__()
        self.input_blocks = unet_model.input_blocks
        self.middle_block = unet_model.middle_block
        self.time_embed = unet_model.time_embed
        self.model_channels = unet_model.model_channels
        self.dtype = unet_model.dtype
...
class UNet_h_decoder(nn.Module):
    def __init__(self, unet_model):
        super().__init__()
        self.output_blocks = unet_model.output_blocks
        self.out = unet_model.out
        self.model_channels = unet_model.model_channels
        self.dtype = unet_model.dtype

    def forward(self, h, hs, emb, x):
        """
        Args:
            h   : middle_h, the output from encoder
            hs  : list of hidden states from encoder (residuals)
            emb : timestep embeddings
            x   : original noisy image (for dtype match)
        Returns:
            output : final prediction (usually predicted noise)
        """
        for module in self.output_blocks:
            h = th.cat([h, hs.pop()], dim=1)
            h = module(h, emb)
