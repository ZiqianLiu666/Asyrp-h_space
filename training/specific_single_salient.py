...
from utils.model_utils import load_diffusion_model, load_cs_model_specific, load_id_lpips_models
...
class Coach:
			...
			for step, (i, j) in enumerate(zip(self.seq_inv, self.seq_inv_next)):
				...
				if replace:                             
					if self.t_end_edit <= i <= self.t_start_edit:
						...
						middle_h = latent_c + (latent_s_list[step])
		...
		while self.global_step < self.opts.max_steps:
			...
			for batch_idx, ((batch_bg, _), (batch_t, _)) in enumerate(zip(self.train_bg_dataloader, self.train_t_dataloader)):
				...
				for step, (i, j) in enumerate(zip(self.seq_inv, self.seq_inv_next)):
					...
					if self.t_end_edit <= i <= self.t_start_edit:
						latent_c_bg, latent_s_bg, g_c_bg_logits, logit_c_bg, logit_s_bg = self.cs_mlp_net(middle_h_bg, is_bg=True)
						latent_c_t, latent_s_t, g_c_t_logits, logit_c_t, logit_s_t = self.cs_mlp_net(middle_h_t, is_bg=False)

						middle_h_bg = latent_c_bg + latent_s_bg
						middle_h_t = latent_c_t + latent_s_t
	...
	def calc_latent_loss(self, latent_bg_c, latent_bg_s, latent_t_c, latent_t_s, w_bg_pSp, w_t_pSp, \
		...
		# 2) Latent reconstruction distance loss
		if self.opts.lat_lambda > 0:
			loss_bg = F.mse_loss(latent_bg_c + latent_bg_s, w_bg_pSp.detach())
			loss_t  = F.mse_loss(latent_t_c + latent_t_s, w_t_pSp.detach())
